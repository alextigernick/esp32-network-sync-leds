...
**Node roles are determined at runtime** — no compile-time flags. The AP node is the time sync root; STA nodes are followers.
...
| `src/time_sync.c/h` | UDP time sync on port 5001. Root serves current ms timestamp on request; followers poll every 5s and apply RTT-corrected offset to `esp_timer_get_time()` |
...
- **AP IP is fixed at `192.168.4.1`** (lwIP default for AP mode). STA nodes hard-code this as the time sync root address.
